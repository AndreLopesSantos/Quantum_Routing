...
def statistical_test_quantum(G, repetitions, inspector=False):
    ...
    for a in range(repetitions):
        ...
        for match in range(num_nodes**2):
            if scipResult[match] != quantumResult[match]:
                ...
                non_optimal_quantum_results.append(
                    objective_function_result(G, quantumResult))
                break
    ...
    print("The number of equal results between quantum and classic: " +
          str(num_equal_results) + "/" + str(repetitions))
    print("Optimal Result (objective function) with SCIP (classical computation): ",
          optimal_result_classic)
...
def quantum_experiment(G):
    ...
    if os.path.exists(filename):
        ...
        resultsFile.write(
            "Experiment, Number of Nodes, SCIP best, SCIP worst, QBSolv classic, QBsolv Quantum, SCIP best time, SCIP worst time, QB classic time, QB quantum time\n")
        exp_number = 1

    resultsLine = str(exp_number) + "," + str(num_nodes) + "," + str(optimal_result_classic) + "," + str(worst_result_classic) + "," + str(objfunc_classic) + "," + \
        str(objfunc_quantum) + "," + str(scipBest_time) + "," + str(scipWorst_time) + \
        "," + str(qbSolvResult_classic_time) + "," + \
        str(qbSolvResult_quantum_time) + "\n"
...
def quantum_experiment_know_solutions(graphfile):
    ...
    # QBsolv - classical implementation
    qbSolvResult_classic_start = time.time()
    qbSolvResult_classic = traveling_salesman(
        G, inspector=False, qbsolv=True, classic=True, debug=False)
    qbSolvResult_classic_end = time.time()
    qbSolvResult_classic_time = qbSolvResult_classic_end - qbSolvResult_classic_start
    objfunc_classic = best_result(G, qbSolvResult_classic)
    # Result if the Travelling Salesman returned to the first node
    fulltsp_classic = objfunc_classic + \
        G.get_edge_data(*(0, num_nodes-1))['weight']
    print("QBsolv Classic - Complete")

    # QBsolv - Quantum implementation
    qbSolvResult_quantum_start = time.time()
    qbSolvResult_quantum = traveling_salesman(
        G, inspector=False, qbsolv=True, classic=False, debug=False)
    qbSolvResult_quantum_end = time.time()
    qbSolvResult_quantum_time = qbSolvResult_quantum_end - qbSolvResult_quantum_start
    objfunc_quantum = best_result(G, qbSolvResult_quantum)
    # Result if the Travelling Salesman returned to the first node
    fulltsp_quantum = objfunc_quantum + \
        G.get_edge_data(*(0, num_nodes-1))['weight']
    print("QBsolv Quantum - Complete")

    # Write data to results file
    filename = "results_known_solutions.csv"
    if os.path.exists(filename):
        resultsFile = open(filename, "a")
        exp_number = sum(1 for line in open(filename))
    else:
        resultsFile = open(filename, "w")
    ...
    resultsLine = str(exp_number) + "," + sample_str + "," + route_type + "," + str(num_nodes) + "," + best_known_result + "," + str(random_route_result) + "," + str(objfunc_classic) + \
        "," + str(fulltsp_classic) + "," + str(objfunc_quantum) + "," + str(fulltsp_quantum) + \
        "," + str(qbSolvResult_classic_time) + "," + \
        str(qbSolvResult_quantum_time) + "\n"
    resultsFile.write(resultsLine)


def classical_repeated_experiment(graphfile, repetitions):
    sample_str = re.findall("[^\.]+", re.findall("[^/]+", graphfile)[-1])[0]

    print("Classical QBsolv for sample: " + sample_str)
    print("Total number of repetitions: ", str(repetitions))
    ...
    filename = "results_known_solutions_classic_only.csv"

    for calculation in range(repetitions):
        ...
        fulltsp_classic = objfunc_classic + \
            G.get_edge_data(*(0, num_nodes-1))['weight']
        print("Repetition number: " + str(calculation + 1) + " done.")
        ...
        if os.path.exists(filename):
            ...
            resultsFile = open(filename, "w")
            resultsFile.write(
                "Experiment, Sample, Type,  Number of Nodes, Best Know Solution, Random route Solution, QBSolv classic, QBSolv classic full TSP, QB classic time \n")
            exp_number = 1

        resultsLine = str(exp_number) + "," + sample_str + "," + route_type + "," + str(num_nodes) + "," + best_known_result + "," + str(
            random_route_result) + "," + str(objfunc_classic) + "," + str(fulltsp_classic) + "," + str(qbSolvResult_classic_time) + "\n"
        resultsFile.write(resultsLine)
...
quantum_experiment_know_solutions("graphs/symmetric/gr21.tsp")
#classical_repeated_experiment("graphs/symmetric/gr21.tsp", 100)
